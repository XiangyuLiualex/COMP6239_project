...
//根据故事ID显示本故事的所有章节
    ...
    suspend fun tChapterListByStoryId(
        @Body values: Map<String, Any>
    ): Response<ResultData<List<readerTChapter>>>

//根据章节ID显示本章节的所有内容
    @POST("/TChapterContentCtrl/tContentListByChapterId")
    suspend fun tContentListByChapterId(
        @Body values: Map<String, Any>
    ): Response<ResultData<List<readerTContent>>>

//    根据章节ID显示本章节的所有选项
    @POST("/TChapterContentCtrl/tOptionListByChapterId")
    suspend fun tOptionListByChapterId(
        @Body values: Map<String, Any>
    ): Response<ResultData<List<readerTOption>>>

//    根据故事ID和章节名模糊查询章节
    @POST("/TChapterContentCtrl/tChapterByStoryIdAndChapterTitle")
    suspend fun tChapterByStoryIdAndChapterTitle(
        @Body values: Map<String, Any>
    ): Response<ResultData<readerTChapter>>
}
